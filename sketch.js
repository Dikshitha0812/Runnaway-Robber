...
function setup() {
  ...
  ground = createSprite(200,480,1700,20);
  ...
  invisibleGround = createSprite(200,485,400,10);
...
function draw() {
  ...
  if (gameState===PLAY){
    ...
    if((keyDown("space") ||touches.length>0)&& mario.y >= 400) {
      mario.velocityY = -10;
      jumpsound. play()
      touches=[];
    }
   ...
   if (ground.x < 0){
    ...
    spawnClouds();
    spawnObstacles();
    obstaclesGroup.setColliderEach("circle",0,0,20)
...
function spawnObstacles() {
  if(frameCount % 60 === 0) {
    var obstacle = createSprite(camera.x+width/2,450,10,40);
    //obstacle.debug = true;
